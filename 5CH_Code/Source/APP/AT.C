...
{
  {
    NV_CANID_ID, &CanAddress,sizeof(CanAddress),AT_TYPE_INT,AT_ATT_NONE,AT_CONFIG_CANID,NULL
  },
...
uint8 appAtCmd(uint8 *pCmd,uint8 *pRsp,uint8 CmdLen,uint8 *pucRspLen)
{
                ...
                if (0 == strcmp( cfgAtt, atItem_t[i].buf))
                {
                     ...
                     if (cfgVal[0] == '?')
                     {
                         ...
                         if (sAtt >= atItem_t[i].att)
                         {
                             ...
                             else if (AT_TYPE_STRING == atItem_t[i].type)
                             {
                                 *pucRspLen += sprintf((char *)pRsp,"%s:%s",atItem_t[i].buf,(char *)atItem_t[i].value);
                             }
                             else if (AT_TYPE_USER == atItem_t[i].type)
                             {
                                 switch(atItem_t[i].id)
                                 {
                                 default:
                                      break;
                                 }
                     ...
                     else
                     {
                         ...
                         else if (AT_TYPE_STRING == atItem_t[i].type)
                         {
                             ...
                             {
                                 strcpy((char *)atItem_t[i].value,cfgVal);
                             }
                         }
                         else if (AT_TYPE_USER == atItem_t[i].type)
                         {
    ...
    if (CmdLen >= taglen)
    {
        ...
        if (0 == strncmp(tag,(char *)pCmd,taglen))
        {
            ...
            if (Att_value_parse((char *)pCmd,'=',&cfgAtt,&cfgVal))
            {
                if (0 == strncmp(cfgAtt,"DBGLEVEL",strlen("DBGLEVEL")))
                {
                    int level;
                    level = atoi(cfgVal);
                
                    VOS_SetLogLevel(level);
