...
UINT8  Serial_FillRcvBuf(UINT8 ucPort ,UINT8 *pData,UINT16 usLength)
{
    ...
    if (!Serial[ucPort].dcb)
    {
        ...
        Serial_ResetRcvIdleCount(ucPort);

        // check buffer full
#ifdef BUFFER_CHECK 
        usSpace = Serial[ucPort].usRcvRear == Serial[ucPort].usRcvFront ? SERIAL_MAX_RECV_BUFF_LENGTH : (SERIAL_MAX_RECV_BUFF_LENGTH + Serial[ucPort].usRcvRear - Serial[ucPort].usRcvFront) % SERIAL_MAX_RECV_BUFF_LENGTH;

        if (usSpace < SERIAL_MAX_RECV_BUFF_LENGTH / 4)
        {
           Serial[ucPort].usRcvIdleCnt = Serial[ucPort].usRcvIdleTime;
           Serial_CheckRcvIdleTime(ucPort);
        }
#endif  
    }
    else
    {
        (Serial[ucPort].dcb)(ucPort,pData,usLength);
    }
...
{
    Serial[ucPort].usRcvIdleTime = Serial[ucPort].SerialConfig.ucIdleCheckThd ; 
    
    Serial[ucPort].usRcvIdleCnt = 0;
}


int Serial_MoveData(UINT8 ucPort,char *data,UINT16 length)
{
...
void Serial_CheckRcvIdleTime(UINT8 ucPort)
{
    ...
    if (Serial[ucPort].usRcvIdleCnt >= Serial[ucPort].usRcvIdleTime)
    {
        ...
        if ((Serial[ucPort].ucDriverType & IPC_DRIVER))
        {
            OS_MBOX_DATA data = {0};

            OSMboxQuery(Serial[ucPort].mb,&data);
...
void Serial_ResetRcvIdleCount(UINT8 ucPort)
{
    Serial[ucPort].usRcvIdleCnt = 0;
...
{
    UINT8 ucOldDrvType = Serial[ucPort].ucDriverType;
    UINT8 ucDelta =  ucDrvType ^ ucOldDrvType;
    ...
    if ((ucDelta & IPC_DRIVER) && (ucOldDrvType & IPC_DRIVER))
    {
        if (Serial[ucPort].mb) 
        {
            INT8U err;
    ...
    else if ((ucDelta & IPC_DRIVER) && (ucDrvType & IPC_DRIVER))
    {
        if (!Serial[ucPort].mb) 
        {
            ...
            if (!Serial[ucPort].mb)
            {
                Serial[ucPort].ucDriverType &= ~(IPC_DRIVER);
            }
        }        
    }
...
int Serial_Read(INT8U ucPort,char *data,UINT16 length,INT16U tmout)
{
...
{
    Serial[ucPort].usRcvRear    = 0;
    Serial[ucPort].usRcvFront   = 0;
    Serial[ucPort].usRcvIdleCnt = 0;
