...
uint8 rpcBuf[2][SAPP_BUF_SIZE], sbFcs, sbIdx;
uint8 *const sbRxBuf = rpcBuf[0]+1;
uint8 *const sbTxBuf = rpcBuf[1]+1;
uint8 sbLen;
...
uint8 sappItfPort       = 0;
uint8 sappTgtType       = RPC_UART_SOF;
...
{
    uint8 len = 2;
    dat[0] = (CanAddress >> 8) & 0XFF;
    dat[1] = (CanAddress >> 0) & 0XFF;
    return len;
}
 ...
 * @return      None.
 */
static uint8 fillDType(uint8 *dat)
{
    uint8 len = NV_DTYPE_SIZE;
    memcpy(dat,aDevType,len);
    return len;
}
...
{
  //uint16 t16 = BUILD_UINT16(sbRxBuf[SAPP_REQ_ADDR_LSB], sbRxBuf[SAPP_REQ_ADDR_MSB]);
  ...
  case SAPP_CMD_SYS_INIT:
      ...
      break;
  case SAPP_CMD_AT:
      rsp = appAtCmd(&sbRxBuf[RPC_POS_DAT0],&sbTxBuf[SAPP_RSP_STATUS+1],SAPP_PAYLOAD_LEN(sbLen),&len);
      break;
  case SAPP_CMD_DATA:
      {
           // for app to handle
      ...
      //break;
  case SAPP_CMD_CFG:
      rsp = Config_Sapp_Entry(&sbRxBuf[RPC_POS_DAT0],&sbTxBuf[SAPP_RSP_STATUS+1],SAPP_PAYLOAD_LEN(sbLen),&len);
      break;
  case SAPP_CMD_INNER:
      {
        return FALSE;
      ...
      //break;
  default:
      memcpy(&sbTxBuf[SAPP_RSP_STATUS+1],&sbRxBuf[RPC_POS_LEN],(sbLen));
  ...
  sbTxBuf[RPC_POS_DAT0] = rsp; // state
...
uint8 SHZNAPP_SerialBootProc(void)
{
  ...
  case SBL_QUERY_ID_CMD:
      {
          len += fillElecID(&sbTxBuf[RPC_POS_DAT0 + len]);
          len += fillCanID(&sbTxBuf[RPC_POS_DAT0 + len]);
          len += fillDType(&sbTxBuf[RPC_POS_DAT0 + len]);
      }
      break;
  case SBL_QUERY_VERSION_CMD:
      {
        int verlen = strlen(Config_GetVersion());
        sbTxBuf[RPC_POS_DAT0 + len] = verlen;
        len += 1;
        memcpy((char *)&sbTxBuf[RPC_POS_DAT0 + len],Config_GetVersion(),verlen);
        len += (verlen);
      }
      break;
  ...
  case SBL_SET_ADDR_CMD:
      ...
      break;
  case SBL_SET_SERIAL_NO:
  	  {
	  	  Config_SaveSerialNo(&sbRxBuf[RPC_POS_DAT0]);
  	  }
      break;
  case SBL_QUERY_SERIAL_NO:
      {
	      len += Config_GetSerialNo(&sbTxBuf[RPC_POS_DAT0] + len);
  	  }
      break;	  
  default:
      rsp = SAPP_FAILURE;
...
uint8 SHZNAPP_SerialUnknowProc(void)
{
  ...
  sbTxBuf[RPC_POS_LEN] = len; // len for data area (NOT INCLUDE CMD0&CMD1&LEN itself)
  sbTxBuf[RPC_POS_CMD1] |= SAPP_RSP_MASK;
  sbTxBuf[RPC_POS_DAT0] = rsp; // state
...
uint8 SHZNAPP_SerialResp(uint8 ucPort)
{
  ...
  sappTgtType = RPC_UART_SOF; // restore to default
  
  (void)HalUARTWrite(ucPort, sbTxBuf-1, len + RPC_UART_FRAME_OVHD);
...
uint8 SHZNAPP_SerialParse(uint8 ucPort)
{
  ...
  while (HalUARTRead(ucPort, &ch, 1))
  {
    ...
    case rpcSteFcs:
      ...
      if ((sbFcs == ch) 
        /*&& ((sbRxBuf[RPC_POS_CMD0] & RPC_SUBSYSTEM_MASK) == RPC_SYS_APP)*/)
      {
        ...
        //(void)SHZNAPP_SerialResp();  // Send the SB response setup in the sbRxBuf passed to sblProc().
